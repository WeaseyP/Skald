...
AudioProcessor :: struct {
	...
	delay_2_buffer: [96000]f32,
	delay_2_write_index: int,
}
	...
	voice.current_freq += (voice.target_freq - voice.current_freq) * glide_coeff;
		...
		// --- Oscillator Node 1 (Unison/Detune) ---
		{
			...
			for i in 0..<unison_count {
				...
				phase_rads := f32(151.200) * (f32(math.PI) / 180.0);
		...
		// --- Delay Node 2 ---
		{
			delay_samples_2 := int(math.clamp((1.000) * sample_rate, 0, 96000-1));
			read_index_2 := (p.delay_2_write_index - delay_samples_2 + len(p.delay_2_buffer)) % len(p.delay_2_buffer);
			delayed_sample_2 := p.delay_2_buffer[read_index_2];
			p.delay_2_buffer[p.delay_2_write_index] = (node_1_out) + delayed_sample_2 * (0.208);
			p.delay_2_write_index = (p.delay_2_write_index + 1) % len(p.delay_2_buffer);
			node_2_out = (node_1_out) * (1.0 - (0.500)) + delayed_sample_2 * (0.500);
		}
