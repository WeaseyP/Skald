...
export const useAudioEngine = (nodes: Node[], edges: Edge[], isLooping: boolean, bpm: number) => {
    ...
    const createAudioNode = (context: AudioContext, node: Node): AudioNode | null => {
        ...
        switch (node.type) {
            ...
            case 'output':
                // The 'output' node is a special case representing the final destination.
                // We create a GainNode to act as a proxy for the AudioContext.destination
                // to allow for consistent connection/disconnection logic.
    ...
    useDeepCompareEffect(() => {
        ...
        // 2. ADD new nodes and UPDATE existing ones
        nodes.forEach(node => {
            ...
            if (!liveNode) {
                ...
                // Node exists, update its parameters
                const now = context.currentTime;
                const rampTime = 0.02; // Smooth parameter changes
