...
Kick_Voice_State :: struct %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)	active: bool,
	note: u8,
	velocity: f32,
	age: f32,
	current_freq: f32,
...
Kick_Processor :: struct %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)	sample_rate: f32,
	voices: [8]Kick_Voice_State,
	next_voice_idx: int,
	prng: PRNG_State,
}

Kick_init :: proc(p: ^Kick_Processor, sr: f32) %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)	p.sample_rate = sr
	p.prng.state = 12345
}

Kick_note_on :: proc(p: ^Kick_Processor, note: u8, velocity: f32) %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)	voice_idx := -1
	for i in 0..<8 %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)		if !p.voices[i].active {
			voice_idx = i
			break
		}
	}
...
Kick_note_off :: proc(p: ^Kick_Processor, note: u8) %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)	for i in 0..<8 %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)		if p.voices[i].active && p.voices[i].note == note {
			releasing := false
...
Kick_process :: proc(p: ^Kick_Processor) -> f32 %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)	sample_rate := p.sample_rate
	output: f32 = 0.0
	for v_idx in 0..<8 %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)		voice := &p.voices[v_idx]
...
Snare_Drum_Voice_State :: struct %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)	active: bool,
	note: u8,
	velocity: f32,
	age: f32,
	current_freq: f32,
...
Snare_Drum_Processor :: struct %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)	sample_rate: f32,
	voices: [8]Snare_Drum_Voice_State,
	next_voice_idx: int,
	prng: PRNG_State,
}

Snare_Drum_init :: proc(p: ^Snare_Drum_Processor, sr: f32) %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)	p.sample_rate = sr
	p.prng.state = 12345
}

Snare_Drum_note_on :: proc(p: ^Snare_Drum_Processor, note: u8, velocity: f32) %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)	voice_idx := -1
	for i in 0..<8 %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)		if !p.voices[i].active {
			voice_idx = i
			break
		}
...
Snare_Drum_note_off :: proc(p: ^Snare_Drum_Processor, note: u8) %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)	for i in 0..<8 %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)		if p.voices[i].active && p.voices[i].note == note {
			releasing := false
...
Snare_Drum_process :: proc(p: ^Snare_Drum_Processor) -> f32 %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)	sample_rate := p.sample_rate
	output: f32 = 0.0
	for v_idx in 0..<8 %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)		voice := &p.voices[v_idx]
		if !voice.active do continue
...
sax_Voice_State :: struct %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)	active: bool,
	note: u8,
	velocity: f32,
	age: f32,
	current_freq: f32,
...
sax_Processor :: struct %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)	sample_rate: f32,
	voices: [8]sax_Voice_State,
	next_voice_idx: int,
	prng: PRNG_State,
}

sax_init :: proc(p: ^sax_Processor, sr: f32) %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)	p.sample_rate = sr
	p.prng.state = 12345
}

sax_note_on :: proc(p: ^sax_Processor, note: u8, velocity: f32) %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)	voice_idx := -1
	for i in 0..<8 %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)		if !p.voices[i].active {
			voice_idx = i
			break
		}
...
sax_note_off :: proc(p: ^sax_Processor, note: u8) %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)	for i in 0..<8 %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)		if p.voices[i].active && p.voices[i].note == note {
			releasing := false
...
sax_process :: proc(p: ^sax_Processor) -> f32 %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)	sample_rate := p.sample_rate
	output: f32 = 0.0
	for v_idx in 0..<8 %!(MISSING ARGUMENT)%!(MISSING CLOSE BRACE)		voice := &p.voices[v_idx]
		if !voice.active do continue
